...
public:
	...
	Patient(int, int, int, int, int, int);
	~Patient();

	std::string is_pulse_normal(); //проверка показателей пульса относительно утвержденных значений для возрастной категории пациента
	double is_pressure_normal();
	void your_age();
